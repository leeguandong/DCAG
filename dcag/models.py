...
class QwenDoubleStreamAttnProcessor2_0:
    ...
    def __call__(
        ...
        if grag_scale != (0,0) :
            ...
            txt_len, txt_bias_scale, txt_delta_scale = txt_srag_scale[:3]
            img_len, img_bias_scale, img_delta_scale = cond_srag_scale[:3]

            # K-GRAG: Key-space bias/delta reweighting (attention routing)
            ...
            # V-GRAG: Value-space bias/delta reweighting (feature aggregation)
            # 5-tuple format: (len, k_bias, k_delta, v_bias, v_delta)
            if len(cond_srag_scale) > 3:
                img_v_bias = cond_srag_scale[3]
                img_v_delta = cond_srag_scale[4]
                if img_v_bias != 1.0 or img_v_delta != 1.0:
                    cond_val_mean = img_value[:,-1*img_len:,:,:].mean(dim=1)
                    img_value[:,-1*img_len:,:,:] = img_v_bias * cond_val_mean + (img_value[:,-1*img_len:,:,:] - cond_val_mean) * img_v_delta
